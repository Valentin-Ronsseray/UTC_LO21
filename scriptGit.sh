...
if ! grep -q nomFichier .gitignore 2> /dev/null; then
    echo .gitignore nomFichier >> .gitignore
fi
...
case "$operation" in
    "pull")
        git init
        git branch -M main
        git pull "https://ValentinRonsseray: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@github.com/ValentinRonsseray/UTC_${uv}.git" main
        ;;
    "push")
        git add -A
        git commit -m "Obsidian : $current_date"
        git push -u "https://ValentinRonsseray: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@github.com/ValentinRonsseray/UTC_${uv}.git" main
        ;;
    *)
